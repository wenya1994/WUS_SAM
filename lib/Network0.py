...
class Network_interFA_noSpade_noEdge_ODE_slot_channel4(nn.Module):
    ...
    #     try:
    #         # ---- ResNet Backbone ----
    #         self.shared_encoder = timm.create_model(model_name="resnet50", pretrained=imagenet_pretrained, in_chans=3,
    #                                                 features_only=True)
    #     except Exception as e:
    #         print(f"Error loading pretrained model: {e}")
    #         # 处理预训练模型下载失败的情况，例如使用本地模型权重
    #         local_path = 'path/to/local/resnet50.pth'
    #         self.shared_encoder = timm.create_model(model_name="resnet50", pretrained=False, in_chans=3,
    #                                                 features_only=True)
    #         checkpoint = load_state_dict_from_hf(local_path)
    #         self.shared_encoder.load_state_dict(checkpoint['state_dict'])
...
if __name__ == '__main__':
    image = torch.rand(4, 3, 384, 384).cuda()
    model = Network_interFA_noSpade_noEdge_ODE_slot_channel4(channels=128).cuda()
    pred_0, f4, f3, f2, f1 = model(image)
    print(pred_0.shape)
    print(f4.shape)
    print(f3.shape)
    print(f2.shape)

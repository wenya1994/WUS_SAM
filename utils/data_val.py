...
class PolypObjDataset_scribble_noEdge(data.Dataset):
    def __init__(self, image_root, gt_root, scribble_root, trainsize=384):
        ...
        self.images = [image_root + f for f in os.listdir(image_root) if f.endswith('.jpg')]
        self.gts = [gt_root + f for f in os.listdir(gt_root) if f.endswith('.jpg') or f.endswith('.png')]
        self.scribbles = [scribble_root + f for f in os.listdir(scribble_root) if
    ...
    def __getitem__(self, index):
        ...
        scribble = cv2.resize(scribble, (self.trainsize, self.trainsize), interpolation=cv2.INTER_NEAREST).astype(
            np.float32)
...
class PolypObjDataset_scribble_noEdge_3326(data.Dataset):
    def __init__(self, image_root, gt_root, scribble_root, imgName_root, trainsize=384):
        ...
        # >>>>>>>>>>>>>>>>>>>TN3k>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        # self.images = [image_root + f.split('.')[0] + '.jpg' for f in imgname_list]
        # self.gts = [gt_root + f.split('.')[0] + '.png' for f in imgname_list]
        # self.scribbles = [scribble_root + f.split('.')[0] + '.png' for f in imgname_list]

        # >>>>>>>>>>>>>>>>>>>BUSI>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        self.images = [image_root + f.split('.')[0] + '.png' for f in imgname_list]  # BUSI
        self.gts = [gt_root + f.split('.')[0] + '_mask.png' for f in imgname_list]
        self.scribbles = [scribble_root + f.split('.')[0] + '_mask.png' for f in imgname_list]
        # self.edges = [edge_root + f for f in os.listdir(edge_root) if f.endswith('.jpg') or f.endswith('.png')]
    ...
    def __getitem__(self, index):
        ...
        scribble[scribble == 0.0] = -1.0  # -1表示未标注
        scribble[scribble == 2.0] = 0.0  # 0表示背景 1表示前景
        # scribble[scribble >= 127.0] = 1.0  # 0表示背景 1表示前景
        # scribble[scribble < 127.0] = 0.0

        image = self.img_transform(image)
        gt = self.gt_transform(gt)
        scribble = torch.tensor(scribble)

        return image, gt, scribble
...
def get_loader_noEdge(image_root, gt_root, batchsize, trainsize, shuffle=True, num_workers=0, pin_memory=True):
...
def get_loader_noEdge_3326(image_root, gt_root, imgname_root, batchsize, trainsize, shuffle=True, num_workers=0,
                           pin_memory=True):
...
def get_loader_scribble_noEdge(image_root, gt_root, scribble_root, batchsize, trainsize, shuffle=True, num_workers=0,
                               pin_memory=True):
...
def get_loader_scribble_noEdge_3326(image_root, gt_root, scribble_root, imgname_root, batchsize, trainsize,
...
class test_dataset:
    def __init__(self, image_root, gt_root, testsize):
        ...
        # >>>>>>>>>>>>>>>>>>>BUSI>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        self.images = [image_root + f for f in os.listdir(image_root) if f.endswith('.jpg') or f.endswith('.png')]
        self.gts = [gt_root + f for f in os.listdir(gt_root) if f.endswith('.jpg') or f.endswith('_mask.png')]
    ...
    def load_data(self):
        ...
        return image, gt_tensor, name, np.array(image_for_post)
...
def get_loader_Scribble(image_root, scribble_root, batchsize, trainsize, shuffle=True, num_workers=0, pin_memory=True):
